function [perm,hydcon,d20] = get_perm_USBR(input,sed_size,layer)
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:N
        for j = 1:M
            ...
            [val,idx] = min(abs(f2-0.20)); %get index of value closest to 0.10
             if isnan(val)
                d20(i,j) = nan;
             else
                d20(i,j) = xnew(idx); % find x [d10 in mm] at y ~ 0.10
             end
             %{
             figure
    ...
    % USBR
    d20_cm = d20/10; 
    USBR = 0.36.*(d20_cm.^2.3); %in cm/s
    hydcon = USBR/100; %in m/s 
