function [perm,hydcon,d10] = get_perm_hazen(input,sed_size,layer)
...
%   Detailed explanation goes here
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:length(percent_mass(1,1,:))
        subplot(2,3,i)
        a = clip_map(percent_mass(:,:,i), clip_dim);
        clims = [0 1];
        imagesc(a,clims)
        %imagesc(a)
        colormap(flipud(gray(225)));
   ...
   % xnew = linspace(low_dmm, high_dmm, 100);  %sediment size d [mm] - more refined for interpolation
    xnew = linspace(low_dmm, 3, 100);  %sediment size d [mm] - more refined for interpolation

    d10 = nan(N,M);
    for i = 1:N
        for j = 1:M
            f2 = interp1(sed_size, reshape(percent_finer(i,j,:), [1,sed_num]), xnew, 'linear', 'extrap'); % linear interpolation
            f3 = interp1(sed_size, reshape(percent_finer(i,j,:), [1,sed_num]), xnew, 'cubic', 'extrap');
            f4 = interp1(sed_size, reshape(percent_finer(i,j,:), [1,sed_num]), xnew, 'spline', 'extrap');
            f5 = interp1(sed_size, reshape(percent_finer(i,j,:), [1,sed_num]), xnew, 'nearest', 'extrap');
            [val,idx] = min(abs(f2-0.10)); %get index of value closest to 0.10
             ...
             %{
             figure
             scatter(sed_size,squeeze(percent_finer(i,j,:)),'k'); hold on
             %plot(sed_size,squeeze(cumpercent_retained(i,j,:)),'k'); hold on
             plot(xnew,f2,'-',xnew(idx),val,'x');
             plot(xnew,f3,'-',xnew(idx),val,'x');          
             plot(xnew,f4,'-',xnew(idx),val,'x');
             plot(xnew,f5,'-',xnew(idx),val,'x');
             ylim([0,1])
